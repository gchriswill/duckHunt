/**
 * Created by scottwainman on 9/21/15.
 */
...
var Game = (function(){             //Game class
    function Game(){
        ...
        Game.screen = document.querySelector("#canvas");    //static refernece to canvas node in HTML
        Game.ctx = Game.screen.getContext("2d");            //static refernece to canvas context
        ...
        this.loadAssets(["dh_bg.jpg", "dh2.png"]);          //begin loading assets
    }
    ...
    Game.prototype.loadAssets = function(list){             //function to load images
        ...
        (function loadAsset(){                              //recursive function that calls itself for each iamge
            ...
            img.addEventListener("load",function(){         //listen for load event
                ...
                {
                    that.setup();                           //contine on to set() function after all images loaded
                }
            });
        })();
    };
    ...
    Game.prototype.setup = function(){                      //game setup function
        ...
        Game.screen.addEventListener("mousemove", function (e) {    //listen for mousemove event
            ...
            this.mouse.x = x;
            this.mouse.y = y;
        }.bind(this));                                      //maintain proper this context
    ...
    Game.prototype.onClick = function(e){                   //callback function that runs when mouse button clicked
        ...
        this.ducks.forEach(function(duck) {
            if (duck.status == 0 && that.getDistance(duck.x, duck.y, that.mouse.x, that.mouse.y) < 35) {
                ...
                var duck = new Duck(that.ss);                   //instantiate a duck
                duck.x = -30;
                duck.y = Math.random()*200+50;                  //position the y coordinate of the duck
                that.ducks.push(duck);
                Duck.speedOffset+=.1;
            }
        });
    };
...
var Duck = (function(){
    ...
    Duck.speedOffset = 0;   //static variable
    function Duck(img){
    ...
    Duck.prototype.update = function(){
        if(this.status==0) {
            ...
            this.x+=(this.speed+Duck.speedOffset)*this.dir;    //move duck across screen based off it's speed and direction properties
...
var CrossHair = (function(){
    function CrossHair(){
        console.log("cross hair created");
        this.draw();
    }
